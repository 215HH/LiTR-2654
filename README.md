# LiTR-2654

The code is coming soon！



# LiTR-2654 Dataset Analysis
This dataset contains 2,654 image groups, each consisting of three modalities: RGB images, depth images, and thermal images. All images have the same resolution of 640×480. The proportions of different scenes and object categories are shown in the figure below.

![7](https://github.com/user-attachments/assets/1accbfbf-a0bf-41d7-b4f1-8176d087a83f)

This repository hosts a randomly sampled subset of the dataset for demonstration and preliminary experimental preview. For access to the complete dataset, please contact the authors directly. The dataset can be down [百度网盘](https://pan.baidu.com/s/127JnwxfPTDCRP6O0CjeTYw&pwd=kk5f
) 
# Results

VDT-2048：The VDT-2048 results can be down  [here](https://pan.baidu.com/s/1QYEFogPRPUllnIvYtGfvLw&pwd=we7x
)

LiTR-2654：The LiTR-2654 results can be down [here](https://pan.baidu.com/s/1X01bpWUnBUjmJxd-vj4oiQ&pwd=d99a
)



# Evaluation Metrics Toolbox

The Evaluation Metrics Toolbox is available [here](https://github.com/lartpang/PySODMetrics)

# Acknowledgements

Thanks to all the seniors who put in the effort.
